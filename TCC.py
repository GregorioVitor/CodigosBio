...
##nomeando cada boxplot##
rotate = -15      #valor do rotation no nome
plt.xticks([])
if args.N[0]==1:
...
ax3.set_ylabel("GC Ratio")
ax3.set_title("GC Ratio")
...
box =[]
for j in range(0, args.N[0]):
    box.append(ax3.boxplot(con_nuc[j], patch_artist= True, positions = [j], widths = 0.5)) ##plot do boxplot
    if j%2 == 0:
        plt.setp(box[j]["boxes"], facecolor="green") ##cor verde do boxplot 
    else:
        plt.setp(box[j]["boxes"], facecolor="yellow") ##cor amarela do boxplot
